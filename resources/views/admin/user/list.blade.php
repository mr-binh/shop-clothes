...
@section('admin.page-title', 'Quản lý tuyển dụng ')
        ...
        <div class="row">
            <div class="col-lg-6">
                <div class="d-flex justify-content-start">
                    ...
                    <a class="mx-2" href="{{ route('admin.user.show') }}">
            ...
            <div class=" col-lg-6">
                <div class=" d-flex flex-row-reverse bd-highlight">
                    ...
                    <a href="{{ route('admin.user.create') }}" class=" btn btn-primary">Thêm mới người dùng
                    </a>
                </div>
            </div>
        </div>
                ...
                <table class=" table table-hover table-responsive-md text-center">
                    ...
                    @foreach ($user as $index => $key)
                        <tr>
                            ...
                            <td>
                                <img src="{{$key->avatar?Storage::url( $key->avatar):'https://vanhoadoanhnghiepvn.vn/wp-content/uploads/2020/08/112815953-stock-vector-no-image-available-icon-flat-vector.jpg'}}" height="100" width="100">
                            </td>
                            <td>
                                @hasanyrole('super admin')
                                <div data-bs-toggle="tooltip" title="Cập nhật quyền hạn "
                                     class="">
                                    <select class="form-select-role form-select form-select-sm"
                                            aria-label="Small select" name="role" data-id="{{$key->id}}">
                                        @foreach($role as $item)
                                            <option value="{{$item->name}}"
                                                    @if($key->roles->pluck('id')->contains($item->id)) selected @endif>{{$item->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                @endhasrole
                            ...
                            <td>
                                <div data-bs-toggle="tooltip" title="Thao tác " class="btn-group dropstart">
                                    ...
                                    <ul class="dropdown-menu  px-4 ">
                                        <li class="my-3">
                                            <a href="{{ route('admin.coupon.edit', $key->id) }}">
                                        ...
                                        <li class="my-3">
                                            ...
                                            {{--                                                $key->posts->count() == 0)--}}
                                            <form action="{{ route('admin.user.destroy', $key->id) }}"
                                                  method="post">
                                                @csrf
                                                @method('delete')
    ...
    <script>
        ...
        const pageForm = {
            selectChangeStatus: function () {
                $(".form-select-role").on("change", function () {
                    let id = $(this).data("id");
