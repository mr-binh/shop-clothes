...
@section('admin.page-title', 'Quản lý chi tiết sản phẩm ')
@section('admin.content')
    <div class="card card-flush p-4">
        <div class="row">
            <div class="col-lg-6">
                <div class="d-flex justify-content-start">
                    <a href="{{route('admin.product.show')}}" class="pe-3"><h1>Danh sách sản phẩm</h1></a>
                    <a class="mx-2" href="{{ route('admin.product.detail.show',['id'=>$productId]) }}">
            ...
            <div class=" col-lg-6">
                <div class=" d-flex flex-row-reverse bd-highlight">
                    ...
                    <a href="{{ route('admin.product.detail.create',['id'=>$productId]) }}" class=" btn btn-primary">Tạo mới sản phẩm
                    </a>
                </div>
            </div>
        </div>
        <div class="table-responsive p-4 card card-flush ">
            @if (count($productvariant) > 0)
                <table class=" table table-hover table-responsive-md text-center">
                    ...
                    <tbody>
                    @foreach ($productvariant as $index => $key)
                        <tr>
                            <td>
                                {{ $productName->name }}
                            </td>
                            ...
                            <td>
                                <div data-bs-toggle="tooltip" title="Thao tác " class="btn-group dropstart">
                                    ...
                                    <ul class="dropdown-menu  px-4 ">
                                        <li class="my-3">
                                            <a href="{{ route('admin.product.detail.edit', $key->id) }}">
...
{{--                                                $key->posts->count() == 0)--}}
                                                <form action="{{ route('admin.product.detail.destroy', $key->id) }}"
                                                      method="post">
                                                    @csrf
                                                    @method('delete')
...
{{--                                                </div>--}}
                ...
                {{ $productvariant->links('pagination::bootstrap-4') }}
            @else
        ...
        const pageRecruitmentForm = {
            selectChangeStatus: function () {
                ...
                $(".form-select-status").on("change", function () {
                    ...
                    if ($(this).val() == 0) {
                        $.ajax({
                            url: `admin/product/un-status/${id}`,
                            method: "POST",
                            data: {
                                _token: _token,
                            },
                        ...
                        $.ajax({
                            url: `admin/product/re-status/${id}`,
                            method: "POST",
                            data: {
                                _token: _token,
                            },
